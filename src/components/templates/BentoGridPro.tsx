...
export default function BentoGridPro({ data }: TemplateProps) {
  ...
  const colors = theme === 'light' 
    ? { bg: '#ffffff', text: '#1d1d1f', secondary: '#86868b', card: '#f5f5f7', border: '#e0e0e0' }
    : { bg: '#000000', text: '#f5f5f7', secondary: '#98989d', card: '#1c1c1e', border: '#333' };

  return (
    <div style={{ background: colors.bg, color: colors.text, minHeight: '100vh', padding: '32px 16px', fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif', transition: 'all 0.3s' }}>
      {/* Theme Toggle */}
      <button
        onClick={toggleTheme}
        style={{
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '16px' }}>
          {/* Hero Card */}
          <div style={{ background: colors.card, borderRadius: '20px', padding: '32px', border: `1px solid ${colors.border}` }}>
          ...
          {data.projects.map((project) => (
            <div 
              ...
              style={{ background: colors.card, borderRadius: '20px', padding: '24px', border: `1px solid ${colors.border}` }}
            >
