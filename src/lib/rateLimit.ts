...
export async function rateLimitMiddleware(request: NextRequest): Promise<NextResponse | null> {
  ...
  try {
    ...
    // Increment counter
    await cacheService.set(cacheKey, currentCount + 1, Math.ceil(RATE_LIMIT_CONFIG.windowMs / 1000))
    
    // Add rate limit headers to response
    const response = NextResponse.next()
    response.headers.set('X-RateLimit-Limit', RATE_LIMIT_CONFIG.maxRequests.toString())
    response.headers.set('X-RateLimit-Remaining', (RATE_LIMIT_CONFIG.maxRequests - currentCount - 1).toString())
